...
class Sip(HandlerBase, DatagramServer):
    ...
    def parseMessage(self, data):
        ...
        if 'user' in response.keys():
            if 'resp' in response.keys():
                # TODO: session is not available in this scope.
    ...
    def ProxyAuthRequired407(self, params, address, session):
        message = 'SIP/2.0 100 Trying\n' + params['fromaddr'] + params['toaddr'] + params['callid'] + params['cseq'] + \
                  params['via'] + params[
                      'contact'] + 'Content-Length: 0\n\nSIP/2.0 407 Proxy authentication required\n' + params[
                      'fromaddr'] + params['toaddr'] + params['callid'] + params['cseq'] + params['via'] + params[
                      'contact'] + 'Allow: INVITE,BYE,INFO,PRACK,CANCEL,ACK,OPTIONS,SUBSCRIBE,NOTIFY,REGISTER,REFER,UPDATE\nContent-Length: 0\n\n'
        session.transcript_outgoing(message)
        self.socket.sendto(message, address)

    def NotFound404(self, params, address, session):
        message = (
        'SIP/2.0 404 Not found\n' + params['fromaddr'] + params['toaddr'] + params['callid'] + params['cseq'] + params[
            'via'] + params['contact'] + params[
            'expires'] + 'Allow: INVITE,BYE,INFO,PRACK,CANCEL,ACK,OPTIONS,SUBSCRIBE,NOTIFY,REFER,UPDATE,MESSAGE\nSupported: outbound,path\n' +
        params['ua'] + 'Content-Length: 0\n\n')
        session.transcript_outgoing(message)
        self.socket.sendto(message, address)

    def BadRequest400(self, params, address, session):
        message = 'SIP/2.0 400 Bad request\n' + params['via'] + params['maxforwards'] + params['fromaddr'] + params[
            'toaddr'] + params['callid'] + params['cseq'] + params['ua'] + params['contact'] + params['expires'] + \
                  params['allow'] + 'Content-Length:  0'
        session.transcript_outgoing(message)
        self.socket.sendto(message, address)
